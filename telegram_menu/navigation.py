...
class TelegramMenuSession:
    ...
    def __init__(self, api_key: str, start_message: str = START_MESSAGE, persistence_path: str = "") -> None:
        ...
        self.application.add_handler(CallbackQueryHandler(self._selected_inline_button_label))
    ...
    async def _GetLocation_Handler(self, update: Update, context: CallbackContext[BT, UD, CD, BD]) -> None:
        ...
        reverse_geocode_result = gmaps.reverse_geocode((session.location.latitude, session.location.longitude))
        address_components = reverse_geocode_result[0]['address_components']
        session.neighborhood = next(
            (component['long_name'] for component in address_components if
             'administrative_area_level_4' in component['types']),
            ''
        )
        if (session.neighborhood == ''):
            session.neighborhood = next(
                (component['long_name'] for component in address_components if 'neighborhood' in component['types']),
                ''
            )
        session.district = next(
            (component['long_name'] for component in address_components if
             'administrative_area_level_2' in component['types']),
            ''
        )
        session.street = next(
            (component['long_name'] for component in address_components if 'route' in component['types']),
            ''
        )
        if (session.street == ''):
            session.street = next(
                (component['long_name'] for component in address_components if 'street_number' in component['types']),
                ''
            )
    ...
    async def _selected_inline_button_label(self, update: Update, context: CallbackContext[BT, UD, CD, BD]) -> None:
        session = self.get_session(update.effective_chat.id)
        session.selected_buton_label = update.callback_query.data
